...
​		致力于做一款前后分离的个人健康平台，旨在配合热爱网页和健身的朋友们开发一款能够完成卡路里统计，网页计划编写，科学食物搭配，以及方便与其它朋友交流心得的平台。
...
​		健康平台中有需要许多的第三方数据，比如：网页卡路里的消耗、食品卡路里的提供、以及各种健身网页的卡路里消耗。需要调取大量的api接口，数据库设计考虑用户分级还有管理员权限设置。比如：普通用户和vip查看的内容区分以及功能区分。管理分为超级管理员和普通管理员。
